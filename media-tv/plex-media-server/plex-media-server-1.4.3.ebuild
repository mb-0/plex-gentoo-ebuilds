...
pkg_postinst() {
	einfo ""
	elog "Change Log for Version 1.4.3.3433"
	elog ""
    elog "  New:"
    elog "  - (Web) Updated Plex Web to 3.0.1."
    elog ""
    elog "  Fixes:"
    elog ""
    elog "  - (macOS) Automatic updates from 1.4.2 were failing. (#6435)"
    elog "  - (Streaming Brain) Occasional crashes when per-user bandwidth limits were enabled. (#6427)"
    elog "  - (Subtitles) Issues playing soft subtitles with certain clients. (#6386)"
    elog "  - (Database) Performance issues or deadlocks optimizing the database while scanning photo libraries. (#6358)"
    elog "  - (Metadata) Preserve customized text fields when unmatching. (#4999)"
    elog "  - (Metadata) Display issues with cast members for certain shows. (#6117)"
    elog "  - (DLNA) Crash setting preferences when preference string in settings was corrupt. (#6421)"
	einfo ""
